...
%, even when they might be deleted or have changed substantially. Thereby, as long as the intent of the query does not change, our counterfactual query rewriting is robust against updates and deletions of documents, while it can generalize to newly added documents that could not obtain relevance feedback in the past via the rewritten query. 
...
We evaluate our counterfactual query rewriting approaches using the LongEval test collection.% on all available timestamps from July 2022~to August~2023. 
